...
DB* db_open_ex(const char* basedir, uint64_t cache_size)
{
	...
	/*
    *! The new mutex may be used immediately for serializing critical resource
    * readwrite_mutex and readcount_mutex is defiened globally as always in db.h
    */
	pthread_mutex_init(&(self->readwrite_mutex),NULL);
    pthread_mutex_init(&(self->readcount_mutex),NULL);	
    self->readcount=0; // counter for parallel read set to 0
